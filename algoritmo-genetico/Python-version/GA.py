...
class GA:
    def __init__(self, population_size, generations, tourn_N, tourn_K, p_crossover, p_mutation, elitism):
        ...
        self.times_mutation_happened = 0
        self.smaller_fitness = 99999999
    ...
    def getTimesMutationHappened(self):
        print(self.times_mutation_happened)
    ...
    def selection(self, flights_organized):
        ...
        # mutation
        #   mutated_population = self._applyMutation(flights_organized, new_population)
    ...
    def _applyMutation(self, flights_organized, new_population: List[Individual]) -> List[Individual]:
        start = 0
        if self.elitism == True: start = 1

        for individual_index in range(start, len(self.population)):

            mutation_chance = uniform(0, 1)

            # mutates only if chance is lower than value of mutation probability
            if mutation_chance > self.p_mutation:
                continue

            self.times_mutation_happened = self.times_mutation_happened + 1
            ...
            if to_or_from == 0: # mutate returning flight
                ...
                new_population[individual_index].researchers[researcher_index].returning_flight = new_returning_flight
            ...
            if to_or_from == 1: # mutate going flight
                ...
                new_population[individual_index].researchers[researcher_index].going_flight = new_going_flight
    ...
    def debug_fitness(self, individual: Individual):
        return individual.totalFlightCosts() + individual.totalAirportWaitingMinutes()
    
    def _rankPopulation(self, population: List[Individual]):
